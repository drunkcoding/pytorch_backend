...
int
GDSLoader::load(const char* filename, int device_id, void** devPtr_p)
{
  void* devPtr = nullptr;
...
int
PipelineLoader::load(const char* filename, int device_id, void** devPtr_p)
{
  ...
  for (int i = 0; i < num_threads_; i++) {
    ...
    threads_.emplace_back(
        &PipelineLoader::backgroundLoad, this, offset, thread_size);
  }
...
int
PipelineLoader::loadAsync(const char* filename, int device_id, void** devPtr_p)
{
  device_id_ = device_id;
  fd_ = open(filename, O_RDONLY | O_DIRECT);
  if (fd_ < 0) {
  ...
  // block per thread
  size_t bpt = (num_blocks + num_threads_ - 1) / num_threads_;
  // std::cout << "num_blocks: " << num_blocks << ", bpt: " << bpt <<
  // std::endl;
  ...
  for (int i = 0; i < num_threads_; i++) {
    ...
    futures_.emplace_back(std::async(
        std::launch::async, &PipelineLoader::backgroundLoad, this, offset,
        thread_size));
    // threads_.emplace_back(&PipelineLoader::backgroundLoad, this, offset,
    // thread_size);
  }
...
int
PipelineLoader::backgroundLoad(size_t offset, size_t size)
{
  checkCudaErrors(cudaSetDevice(device_id_));
  ...
  while (size > 0) {
    ...
    // copy data to GPU
    checkCudaErrors(
        cudaMemcpy(devPtr_ + offset, buf, ret, cudaMemcpyHostToDevice));

    // update offset and size
    offset += ret;
    size -= ret;
    // std::cout << "offset: " << offset << ", size: " << size << ", ret: " <<
    // ret << std::endl;
  }
...
int
PipelineLoader::backgroundRead(size_t offset, size_t size)
{
  while (size > 0) {
...
PipelineLoader::backgroundCopy()
{
  ...
  while (copied_size < file_size_) {
    ...
    checkCudaErrors(cudaMemcpy(
        devPtr_ + t->offset, t->buffer, t->size, cudaMemcpyHostToDevice));
    copied_size += t->size;
...
int
PipelineLoader::readCopy(const char* filename, int device_id, void** devPtr_p)
{
  ...
  for (int i = 0; i < num_threads_; i++) {
    ...
    threads_.emplace_back(
        &PipelineLoader::backgroundRead, this, offset, thread_size);
  }
